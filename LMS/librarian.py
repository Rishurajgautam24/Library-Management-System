...
def dashboard():
    if current_user.role ==1:
        if request.method == 'GET':
            ...
            # book count,issued books,requested books 
            ...
            if request.args.get('message'):
                ...
                if request.args.get('type'):
                    return render_template("librarian/dashboard.html", user=current_user, data=data, book_count=book_count, issued_books=issued_books, requested_books=requested_books, message=message, type="success")
                return render_template("librarian/dashboard.html", user=current_user, data=data, book_count=book_count, issued_books=issued_books, requested_books=requested_books, message=message)
    ...
    if current_user.role ==1:
        if request.method == 'GET':
            ...
            if request.args.get('message'):
                message = request.args.get('message')
                if request.args.get('type'):
                    return render_template("librarian/books/view_books.html", user=current_user, books=data, message=message, type="success")
                return render_template("librarian/books/view_books.html", user=current_user, books=data, message=message)
...
def add_book():
    if current_user.role ==1:
        ...
        if request.method == "POST":
            ...
            new_book = Book(name=name, author=author,isbn=isbn, content=content, section_id=section_id)
    ...
    if current_user.role ==1:
        if request.method == 'GET':
            ...
            if request.args.get('message'):
                message = request.args.get('message')
                if request.args.get('type'):
                    return render_template("librarian/sections/view_sections.html", user=current_user, sections=data, message=message, type="success")
                return render_template("librarian/sections/view_sections.html", user=current_user, sections=data, message=message)
...
def add_section():
    if current_user.role ==1:
        if request.method == "GET":
            ...
            return render_template("librarian/sections/add_section.html")
        if request.method == "POST":
    ...
    if current_user.role ==1:
        if request.method == 'GET':
            ...
            if request.args.get('message'):
                message = request.args.get('message')
                if request.args.get('type'):
                    return render_template("librarian/users/view_users.html", user=current_user, users=data, message=message, type="success")
                return render_template("librarian/users/view_users.html", user=current_user, users=data, message=message)
        ...
        if request.method == "POST":
            ...
            if request.form.get("password"):
                ...
                if password != confirm_password:
                    return render_template("librarian/users/edit_user.html", message="Passwords do not match")
            role = request.form.get("role")
        ...
        if request.method == 'GET':
            ...
            for req in requests_book:
                if req.id != requested.id:
                    req.status = "Rejected"
            # make a record in transaction table for the approved request
...
def search_book():
    if current_user.role ==1:
        if request.method == 'POST':
            ...
            if books:
                return render_template("librarian/search_book.html",books=books,user=current_user)
            return redirect(url_for('librarian.dashboard',message="No books found",type="danger",user=current_user))
...
def search_user():
    if current_user.role ==1:
        if request.method == 'POST':
            ...
            if users:
                return render_template("librarian/search_users.html",users=users,user=current_user)
            return redirect(url_for('librarian.dashboard',message="No users found",type="danger",user=current_user))
...
def search_section():
    if current_user.role ==1:
        if request.method == 'POST':
            ...
            return redirect(url_for('librarian.dashboard',message="No sections found",type="danger",user=current_user))
        
# auto revoke the book if not returned in time 
...
# for giving permission to download book 
...
def download(id):
    if current_user.role ==1:
        ...
        permission = True
        new_download = Download(book_id=book_id, user_id=user_id, permission=permission)
